...
- [x] **FND-06**: `MEXCSpotClient` class (read-only methods wired first) using CCXT 4.5.48+, with base URL `api.mexc.com` exposed as config, not hardcoded — satisfied by Plan 01-04 (commit `84b8c17`), 5 unit tests + 2 live tests gated on MEXC_LIVE=1; **live ping verified 2026-04-18 22:15 via `pnpm smoke` — real call to api.mexc.com returned serverTime**
- [x] **FND-07**: `MEXCFuturesClient` class stub (read-only methods only in v1) using CCXT, separate auth + rate bucket from spot client, base URL `contract.mexc.com` config-driven — satisfied by Plan 01-04 (commit `ffbc15a`), 6 unit tests incl. cross-package instance-separation; **live ping verified 2026-04-18 22:15 via `pnpm smoke` — real call to contract.mexc.com returned serverTime (after fixing MexcFuturesPingSchema to coerce string→number for MEXC's actual response shape)**
- [x] **FND-08**: Boot-time smoke test pings both MEXC endpoints (spot ping + futures ping) and fails fast with a clear error if either is unreachable — satisfied by Plan 01-05 (commit `408eef3`). **Both branches verified live 2026-04-18:** pre-flight fail path (16:41, exit 1, all 3 missing secrets listed in one fatal log) AND full happy path (22:15, exit 0, both MEXC pings OK + IP whitelist match + `Phase 1 boot complete - all systems ready`). See `docs/phase-1-readiness.md` §3 for the full log trail.
