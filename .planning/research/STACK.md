...
**Domain:** Personal crypto trading copilot (MEXC spot + USDT-M futures, local CPU-only ML, semi-auto via Telegram)
**Researched:** 2026-04-17
**Confidence:** MEDIUM-HIGH overall — HIGH on runtime/ecosystem, MEDIUM on MEXC SDK choice (see "CCXT vs native" callout), HIGH on the ML split, MEDIUM on news APIs (CryptoPanic is alive but rate-capped)

## Executive Summary

The 2026 stack for this project is **Node.js 22 LTS + TypeScript + CCXT 4.5.x** for all MEXC access (spot + the newly-public futures API), **Python (separate process) for XGBoost/LightGBM training**, **ONNX Runtime in Node for inference**, **Redis + BullMQ** for state and queues, **better-sqlite3** for the trade ledger, **grammY** for Telegram, **Fastify + Vite/React + lightweight-charts** for the web dashboard, **Ink (React-in-terminal)** for the CLI, **@zowe/secrets-for-zowe-sdk** for Windows Credential Manager access, and **PM2 + pm2-windows-service** on Windows / **PM2 + systemd** on Hostinger. Solana parsing uses **Helius `getTransactionsForAddress`** on top of **@solana/kit** (web3.js v2, renamed Dec 2024). Ethereum uses **viem** + **Alchemy** for enriched wallet history.

The single most important finding for this project: **MEXC opened its Futures API to all KYC'd retail users on March 31, 2026**. Before that date it was institutional-only. Any stack advice from before April 2026 that says "MEXC futures API requires an institutional account" is obsolete. Maker/taker fees for API futures are 0.01% / 0.05% (separate from web/app rates). ([MEXC announcement](https://www.mexc.com/announcements/article/introducing-api-futures-trading-on-mar-31-2026-17827791534551))

The second most important finding: **the official MEXC SDK (`mexc-api-sdk`) is abandoned** — last meaningful update ~2 years ago, has open signature-calculation bugs as recent as Dec 2025 (issues #87, #112), and is not a safe foundation. CCXT 4.5.x is the pragmatic default; native MEXC calls with `node:crypto` HMAC-SHA256 are the fallback for endpoints CCXT doesn't unify well. Do not use `mexc-api-sdk` or any of its un-maintained forks as the primary client.
...
pnpm init
pnpm add -D typescript@5.7 tsx@4.19 tsup@8.3 vitest@2.1 @biomejs/biome@1.9 @types/node@22

# Core runtime
pnpm add ccxt@4.5 zod@3.23 pino@9 pino-pretty dotenv @t3-oss/env-core
...
pnpm add ioredis@5 bullmq@5 better-sqlite3@11

# On-chain
pnpm add @solana/kit @solana/compat helius-sdk viem@2 alchemy-sdk@3

# ML inference (Node-side; training is Python-side in separate folder)
pnpm add onnxruntime-node@1.22

# Technical indicators (live updates in Node; offline batch uses Python pandas-ta)
pnpm add technicalindicators@3

# Telegram
pnpm add grammy@1

# Web dashboard
pnpm add fastify@4 @fastify/websocket @fastify/static @fastify/cors
# (in apps/web-dashboard) - Vite + React setup
pnpm create vite@latest apps/web-dashboard -- --template react-ts
pnpm add -F web-dashboard lightweight-charts@4

# CLI dashboard
pnpm add ink@5 @inkjs/ui react

# Secrets (Windows Credential Manager / macOS Keychain / libsecret on Linux)
pnpm add @zowe/secrets-for-zowe-sdk
...
pip install "xgboost>=2.1" "lightgbm>=4.5" "pandas>=2.2" "polars>=1" \
            "scikit-learn>=1.5" "pandas-ta>=0.3.14b" \
            "onnx>=1.17" "onnxmltools>=1.13" "skl2onnx>=1.18" \
            "jupyterlab>=4" "joblib>=1.4"
```
...
1. **Two separate base URLs, two separate auth schemes.**
   - Spot V3: `https://api.mexc.com` — signature is HMAC-SHA256 of the full query string with `ApiKey`/`Request-Time`/`Signature` headers.
   - Futures/Contract: `https://contract.mexc.com` — different header names and slightly different signature payload format. ([MEXC futures integration guide](https://www.mexc.com/api-docs/futures/integration-guide))
   - MEXC **enabled a new Futures API access domain on Jan 12, 2026** — signature logic unchanged but double-check your base URL matches the current doc page.

2. **Futures API was institutional-only until March 31, 2026.** It is **now public for all KYC users**. Your API key must be generated with Futures permission explicitly enabled in the MEXC web UI — existing spot-only keys will return 403.

3. **API Futures fee schedule is different from web/app.** Maker 0.01% / Taker 0.05%. Do not assume parity with web fees in PnL math.

4. **IP binding** — MEXC API keys without a bound IP expire after 90 days. Bind the Windows machine's IP AND the Hostinger VPS IP to the same key (MEXC allows multiple). Otherwise you'll find your key dead on day 91 mid-trade.

5. **Spot order rate limit was tightened Mar 25, 2025** to 12 req/sec total across order-place + order-cancel (shared UID bucket). Batch orders are 2 req/sec for 20-symbol batches. Design signal cadence accordingly.

6. **Minimum spot order size**: 0.5 USDT notional minimum per pair (confirmed by error messages; exact per-pair `minNotional` should be fetched from `/api/v3/exchangeInfo` at startup). At $10 total bankroll this matters for sizing logic — many alts will have higher min-notional, constraining you to BTC/ETH/SOL/USDT pairs.

7. **Minimum futures contract size**: ETH_USDT contract notional is lowest of BTC/ETH/SOL — one contract is typically 0.01 ETH. Check each contract's `minVol` from `/api/v1/contract/detail` at startup. At $10 + 3x max leverage on ETH, you're limited to very small position counts.

8. **Order types supported**: LIMIT, MARKET, LIMIT_MAKER, IMMEDIATE_OR_CANCEL on spot. Futures supports additional types (post-only, FOK). **MEXC does not support OCO orders** — you must manage stop + target as two separate orders, cancelling the other when one fills. Plan for this in the risk manager.

9. **WebSocket connection**: MEXC contract depth is pushed every 200ms after subscribe. Don't assume real-time tick-by-tick updates. Build signal logic tolerant to 200ms bucketed updates.

10. **Recv-Window**: default 5000ms. Windows laptop wake-from-sleep can leave the clock skewed; always force `w32tm /resync` on wake and verify before the first post-wake order.

11. **Known CCXT + MEXC issue**: slow orderbook WS startup on low-volume pairs ([ccxt#18281](https://github.com/ccxt/ccxt/issues/18281)). BTC/ETH/SOL are fine; if you expand to alts, expect cold-start warnings.

12. **Known MEXC broker-endpoint signature bug** (even in CCXT) — for POST/PUT/DELETE to broker endpoints, `timestamp`/`recvWindow`/`signature` must be in the URL while other params are in the body ([ccxt#24970](https://github.com/ccxt/ccxt/issues/24970)). If you hit a signature error on a POST, check this first.
...
**Recommendation: CCXT as the default; native with `node:crypto` HMAC as a pre-built escape hatch.** Build a thin wrapper module `src/mexc/client.ts` that exposes the high-level operations your bot needs (`placeOrder`, `cancelOrder`, `getBalance`, `getRecentTrades`, `subscribeTrades`, `subscribeOrderbook`) and use CCXT under the hood. When (not if) you hit a CCXT quirk on a specific endpoint, swap that one method to native — all your call sites stay untouched. This is the only sane architecture given the state of MEXC's SDK ecosystem.

## Windows Credential Manager Integration

**Recommended approach**: Use `@zowe/secrets-for-zowe-sdk` (the keytar drop-in replacement). On Windows 11, it writes via the native Windows Credential Manager (Credential Vault / `wincred.dll`) through a prebuilt native binding — no PowerShell bridge needed.

```typescript
import { keyring } from "@zowe/secrets-for-zowe-sdk";

const SERVICE = "kr8tiv-mexc-bot";

// Store
await keyring.setPassword(SERVICE, "MEXC_SPOT_API_KEY", "<key>");
await keyring.setPassword(SERVICE, "MEXC_SPOT_API_SECRET", "<secret>");
await keyring.setPassword(SERVICE, "MEXC_FUTURES_API_KEY", "<key>");
await keyring.setPassword(SERVICE, "MEXC_FUTURES_API_SECRET", "<secret>");
await keyring.setPassword(SERVICE, "TELEGRAM_BOT_TOKEN", "<token>");
// ... wallets, etc.

// Retrieve at startup
const mexcSpotKey = await keyring.getPassword(SERVICE, "MEXC_SPOT_API_KEY");
```

Provide a one-off CLI setup script (`npm run setup:credentials`) that prompts for each secret interactively and stores them — never hardcode, never `.env`-commit. On the VPS, same API surface writes to libsecret/gnome-keyring.

**Why NOT the PowerShell bridge approach** (`child_process` invoking `Get-Credential` / `Get-CredentialManager`): (1) breaks on the Hostinger VPS; (2) PowerShell spawn is ~200ms per call vs <5ms native; (3) adds a dependency on `CredentialManager` PS module installation; (4) error handling across `child_process` boundary is painful. Zowe native binding is strictly better.
...
- **Install order**:
  1. SSH key, disable password auth, `ufw default deny incoming`, allow 22.
  2. `curl -fsSL https://deb.nodesource.com/setup_22.x | sudo -E bash -` → `apt install nodejs`
  3. `apt install redis-server build-essential python3-venv`
  4. `npm install -g pm2 && pm2 install pm2-logrotate`
  5. Deploy via `git clone` + `pnpm install --prod` + build step; `pm2 start ecosystem.config.js && pm2 save && pm2 startup systemd`
...
---
*Stack research for: Personal crypto trading copilot on MEXC (spot + USDT-M futures), CPU-only ML, Node+TS core, Python ML pipeline, Windows primary + Hostinger VPS backup.*
*Researched: 2026-04-17*
